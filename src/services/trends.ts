...
export class TrendsService extends Service {
  async getTrendingSearches() {
    const settings = await prisma.settings.findUniqueOrThrow({ where: { id: 1 } })
    if (settings.manualTrends) {
      const trends = settings.trends as unknown as string[]
      return trends
    }

    const start = await this.getTrendStartDate(settings)
  ...
  async getTrendStartDate(settings: any) {
   ...
   */
  async getRelatedSearches(searchText: string, currentUserId: string) {
    const searchTimelines = await this.getSuccessfulSearchTimelines(searchText, currentUserId)
   ...
   */
  async getSuccessfulSearchTimelines(searchText: string, currentUserId: string) {
